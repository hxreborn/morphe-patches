...
static void trap_kill_sites(uint32_t *base) {
    if (!image_limit || (uintptr_t)base >= image_limit) return;

    static uint32_t window[SCAN_CHUNK + 2];
    ...
    while (at + 3 <= total) {
        ...
        if (want > SCAN_CHUNK + 2) want = SCAN_CHUNK + 2;
        ...
        for (size_t index = 0; index + 2 < want; index++) {
            if (window[index] != MOV_W1_9 || window[index + 1] != MOV_X8_KILL ||
                window[index + 2] != SVC_0) {
                continue;
            }

            uint32_t *site = base + at + index + 1;
            if (!unprotect(site)) continue;

            *(volatile uint32_t *)site = BRK_1;
            reprotect(site);
            trapped++;
        }
    ...
    LOG("trapped %zu SIGKILL sites", trapped);
}
